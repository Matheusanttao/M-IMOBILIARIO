...
5. Copie `.env.example` para `.env.local` e preencha:
   ...
   - Opcional: Mercado Pago, OpenAI, WhatsApp (ver `.env.example`)
...
- `POST /api/webhooks/mercadopago` — webhook (configurar URL e secret no painel MP)
- `POST /api/mercadopago/checkout` — cria assinatura MP e persiste `mercadopago_subscription_id` em `assinaturas`
...
- Edge function de referência: `supabase/functions/mercadopago-webhook/` (pode encaminhar para a rota Next acima)

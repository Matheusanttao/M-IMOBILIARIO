...
declare
  v_user_id uuid;
  v_email text := 'matheusantaosilva18@gmail.com';
  v_password text := '200220';
  v_empresa_id uuid;
...
begin
  ...
  if v_user_id is null then
    v_user_id := 'f7a0d09e-9f6c-4c0c-a78f-6189d1ef9281';
    ...
    values (
      ...
      '{"full_name":"Matheus Antao Silva"}'::jsonb,
      now(),
      now()
    );
  else
  ...
  if not exists (
    ...
    where provider = 'email' and provider_id = v_user_id::text
  ) then
    select data_type
    ...
    if v_identity_id_type = 'uuid' then
      execute $sql$
        ...
        values ($1, $1, $2, jsonb_build_object('sub', $2, 'email', $3), 'email', now(), now(), now())
      $sql$ using v_user_id, v_user_id::text, v_email;
    else
      execute $sql$
        insert into auth.identities (
          id, user_id, provider_id, identity_data, provider, last_sign_in_at, created_at, updated_at
        )
        values ($1, $2, $1, jsonb_build_object('sub', $1, 'email', $3), 'email', now(), now(), now())
      $sql$ using v_user_id::text, v_user_id, v_email;
    end if;
  end if;
  ...
  if v_empresa_id is not null then
    ...
    values (v_user_id, v_empresa_id, 'Matheus Antao Silva', v_email, 'admin', true)

...
export default function MasterDashboardPage() {
  ...
  const loadData = useCallback(async () => {
    ...
    const revenueMonths: RevenueData[] = []
    for (let i = 5; i >= 0; i--) {
      const d = new Date(now.getFullYear(), now.getMonth() - i, 1)
      const label = d.toLocaleDateString('pt-BR', { month: 'short', year: '2-digit' })
      const start = d.toISOString()
      const end = new Date(d.getFullYear(), d.getMonth() + 1, 1).toISOString()
      const total = (pagamentosData ?? [])
        .filter((p) => p.created_at >= start && p.created_at < end)
        .reduce((s, p) => s + Number(p.valor), 0)
      revenueMonths.push({ month: label, mrr: total })
    }
  ...
  const kpis = [
    ...
    { label: 'MRR', value: formatCurrencyBRL(mrr), color: 'text-blue-600' },
    {
      label: 'Inadimplentes',
      value: inadimplentes,
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-6 shadow-md">
          ...
          <ResponsiveContainer width="100%" height={260}>
            <LineChart data={revenueData}>
              ...
              <Tooltip
                contentStyle={{ borderRadius: 12, border: '1px solid #e2e8f0' }}
                formatter={(v: number) => [formatCurrencyBRL(v), 'Receita']}
              />
              <Line
                type="monotone"
                dataKey="mrr"

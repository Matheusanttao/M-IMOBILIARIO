  ...
  const fields = (
    <div className="space-y-4 [&_input]:border-white/10 [&_input]:bg-white/[0.04] [&_input]:text-white [&_input]:placeholder:text-white/35 [&_label]:!text-white/70 [&_option]:bg-card [&_option]:text-white [&_select]:border-white/10 [&_select]:bg-white/[0.04] [&_select]:text-white">
      ...
      <Button
        type="button"
        variant="secondary"
        className="w-full border-white/15 bg-white/5 text-white hover:border-accent hover:bg-white/10 hover:text-accent"
        onClick={() => {
          onChange({})
          onApply?.()
        }}
  ...
  return (
    <>
      <div className="hidden lg:block">
        <div className="sticky top-24 rounded-2xl border border-white/10 bg-card p-6 shadow-xl shadow-black/20">
      ...
      <Modal
        ...
        title="Filtros"
        className="max-w-lg border border-white/10 bg-card text-white [&_h2]:!text-white"
      >
        {fields}
      </Modal>
    </>
